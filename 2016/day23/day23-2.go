...
func solution(context *cli.Context) (result int) {
	...
	data.Registers["a"] = context.Int("a")
	data.Registers["b"] = 0
	data.Registers["c"] = 0
	data.Registers["d"] = 0
	for pc := 0; pc < len(data.Instructions); {
		...
		case Dec:
			data.Registers[op.Operand1.Register]--
		case Jnz:
			v1 := data.Registers[op.Operand1.Register]
			v2 := data.Registers[op.Operand2.Register]
			if op.Operand1.Register == "" {
				v1 = op.Operand1.Value
			}
			if op.Operand2.Register == "" {
				v2 = op.Operand2.Value
			}
			if v1 != 0 && v2 != 0 {
				pc += v2
			} else {
		...
		case Tgl:
			log.Info(fmt.Sprintf("Applying operation at PC[%d] (%#v) to %#v", pc, op, data.Registers))
			v := data.Registers[op.Operand1.Register]
			if (pc+v) > 0 && (pc+v) < len(data.Instructions) {
				data.Instructions[pc+v].Toggle()
			}
		case Mul:
			data.Registers[op.Operand1.Register] *= data.Registers[op.Operand2.Register]
		case Add:
			data.Registers[op.Operand1.Register] += data.Registers[op.Operand2.Register]
		}
		if opType != Jnz {
