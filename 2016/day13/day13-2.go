...
func solution(context *cli.Context) (result int) {
	...
	for i := 0; i <= max+2; i++ {
		for j := 0; j <= max+2; j++ {
			...
			target := Coord{j, i}
			if m.Get(target) {
				log.Info(fmt.Sprintf("Target (%d, %d) is blocked", target.X, target.Y))
			} else {
				log.Debug(fmt.Sprintf("Looking for path to (%d, %d) with designer number %d", target.X, target.Y, m.Designer))
				log.Debug(fmt.Sprintf("Initial map setup is:\n%s", m.Print(target, []Coord{})))
				path := astar.FindPath[Coord](&m, m.Current, target, cost, heuristic)
				if path == nil {
					log.Info(fmt.Sprintf("No path found for target (%d, %d)", target.X, target.Y))
				} else if (len(path) - 1) <= max {
					result += 1
					log.Info(fmt.Sprintf(
						"Found path to (%d, %d) with length %d less or equal to %d",
						target.X,
						target.Y,
						len(path)-1,
						max,
					...
					log.Info(fmt.Sprintf(
						"Found path to (%d, %d) with length %d greater to %d",
						target.X,
						target.Y,
						len(path)-1,
						max,
