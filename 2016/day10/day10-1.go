...
func solution(context *cli.Context) (result string) {
	...
	targetLow := 17
	if context.NArg() == 2 {
		targetHigh = atoi(context.Args().Get(0))
		targetLow = atoi(context.Args().Get(1))
	}
	...
	log.Info(fmt.Sprintf("Looking for bot with values (%d, %d) with %d outputs", targetLow, targetHigh, len(data.Outputs)))
	...
	for i := 0; i < len(data.Outputs); i++ {
		outputEdge := data.Outputs[strconv.Itoa(i)]
		log.Info(fmt.Sprintf("Found value %d for %d (%#v) output", outputEdge.HighValue, i, outputEdge))
	}
	for k, v := range data.Bots {
		if v.LowValue == targetLow && v.HighValue == targetHigh {
			result = k
		}
	}
	return
