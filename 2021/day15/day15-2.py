...
@lru_cache(maxsize=10 * 1024 * 1024)
...
def main(argv=None):
    ...
    x_size, y_size = max(x for x, _ in mapa.keys()) + 1, max(y for _, y in mapa.keys()) + 1

    logger.info('Read map, %s X %s', x_size, y_size)

    for x in range(x_size * 5):
        for y in range(y_size * 5):
            if (x, y) not in mapa:
                x_offset = x // x_size
                y_offset = y // y_size
                mapa[(x, y)] = mapa[(x % x_size, y % y_size)] + x_offset + y_offset
                if mapa[(x, y)] > 9:
                    mapa[(x, y)] -= 9
                logger.debug(
                    'Generating (%s, %s) from (%s, %s) * %s. %s --> %s',
                    x, y, x % x_size, y % y_size, x_offset + y_offset,
                    mapa[(x % x_size, y % y_size)], mapa[(x, y)]
                )

    path = {}

    start_x, start_y = 0, 0
    x_size, y_size = x_size * 5, y_size * 5

    with open('{}.big'.format(args.input.name), 'w') as f:
        for x in range(x_size):
            for y in range(y_size):
                print(mapa[(x, y)], end="", file=f)
        print("", file=f)

    logger.info('Generated new input at %s', '{}.big'.format(args.input.name))

    path = a_star(mapa, (0, 0), (x_size - 1, y_size - 1))
