...
def main(argv=None):
    ...
    arg_parser.add_argument('-s', '--steps', type=int, default=100,
    ...
    for i in range(args.steps):
        ...
        logger.debug('Octopuses at step %s before recursivity', i + 1)
        logger.debug('\n'+print_octopuses(octopuses))
        for x, y in flashed.copy():
            logger.info('Octopus %s flashed at step %03d of %03d', (x, y), i + 1, args.steps)
        ...
        logger.debug('Octopuses at step %s after recursivity', i + 1)
        logger.debug('\n'+print_octopuses(octopuses))
