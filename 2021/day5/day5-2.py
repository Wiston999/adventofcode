...
class Line(object):
    ...
    def points(self):
        if self.start_x == self.end_x:
            ...
            length = abs(self.start_x - self.end_x)
            logger.debug('Yielding %s points for line %s', length, self)
            for step in range(length + 1):
                yield (
                    self.start_x + (step if self.start_x < self.end_x else -step),
                    self.start_y + (step if self.start_y < self.end_y else -step)
                )
