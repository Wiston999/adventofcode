...
func (p *Problem) SolutionAStar(steps int) (path []State, score int) {
	start := State{p.Start, steps}
...
func solution(context *cli.Context) (result int) {
	...
	path, _ := problem.SolutionAStar(0)
	result += len(path)
	log.Info(fmt.Sprintf("Reached target [%v] in %d steps (%04d)", problem.Target, len(path), result))
	problem.Start = target
	problem.Target = start
	path, _ = problem.SolutionAStar(result)
	result += len(path)
	log.Info(fmt.Sprintf("Reached target [%v] in %d steps (%04d)", problem.Target, len(path), result))
	problem.Start = start
