...
func solution(context *cli.Context) (result int) {
	...
	log.Info("Computed Flows")
	for i := 0; i < (1 << len(problem.Keys)); i++ {
		for j := 0; j < (1 << len(problem.Keys)); j++ {
			if i&j != j {
				continue
			}
			a, b := -9999, -9999
			for k := 0; k < len(problem.Keys); k++ {
				if a < problem.DP[problem.Limit][k][j] {
					a = problem.DP[problem.Limit][k][j]
				}
				if b < problem.DP[problem.Limit][k][i&^j] {
					b = problem.DP[problem.Limit][k][i&^j]
				}
			}
			if result < (a + b) {
				result = a + b
