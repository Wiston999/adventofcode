...
func (n *Node) FindBest(needed, best int) (result int) {
	result = best
	if n.Size == 0 {
		usage := n.Usage()
		if usage >= needed && usage < best {
			result = usage
		}
		for _, v := range n.Children {
			result = v.FindBest(needed, result)
		}
	}
	return
}
...
func solution(context *cli.Context) (result int) {
	...
	freeSpace := 70000000 - problem.Tree.Usage()
	neededSpace := 30000000
	log.Info(fmt.Sprintf("Free space %06d", freeSpace))
	result = problem.Tree.FindBest(neededSpace-freeSpace, neededSpace)
