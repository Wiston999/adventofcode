...
func solution(context *cli.Context) (result int) {
	...
	result = 9999999
	for c, p := range problem.Tiles {
		if p == 0 {
			path := astar.FindPath[Coord](&problem, c, problem.Target, cost, heuristic)
			log.Info(fmt.Sprintf("Searched path from %v: %d", c, len(path)-1))
			if len(path) > 0 && (len(path)-1) < result {
				result = len(path) - 1
			}
		}
	}
