...
def is_nice(data):
    ...
    result = True
    result = result and sum(data.count(c) for c in 'aeiou') >= 3
    result = result and any(data[i] == data[i+1] for i in range(len(data) - 1))
    result = result and not any(f in data for f in forbidden)
    return result
