...
func (s Sequence) Interpolate(forward bool) (result int) {
	tmp := make(Sequence, len(s)-1)
	allZero := true
	for i := 1; i < len(s); i++ {
		tmp[i-1] = s[i] - s[i-1]
		if tmp[i-1] != 0 {
	...
	if allZero {
		...
		if forward {
			result = baseValue + tmp.Interpolate(forward)
		} else {
			result = baseValue - tmp.Interpolate(forward)
		}
	}
...
func (p *Problem) Part1() (result int) {
	for i, s := range p.Sequences {
		log.Debug(fmt.Sprintf("[%03d] Interpolating %v", i, s))
		result += s.Interpolate(true)
	}
	return
}
...
func NewProblem(ctx *cli.Context) (p *Problem, err error) {
	...
	for i, l := range strings.Split(strings.TrimSpace(strData), "\n") {
		...
		p.Sequences = append(p.Sequences, s)
	}
	return
}
