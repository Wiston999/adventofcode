...
type Generator struct {
	Current, Factor, Criteria uint
}

func (g *Generator) Next() uint {
	g.NextRaw()
	for (g.Current % g.Criteria) != 0 {
		g.NextRaw()
	}
	return g.Current
}

func (g *Generator) NextRaw() uint {
	g.Current *= g.Factor
	g.Current %= MODULUS
	return g.Current
}
...
func solution(context *cli.Context) (result int) {
	...
	A, B := Generator{uint(context.Int("a")), 16807, 4}, Generator{uint(context.Int("b")), 48271, 8}
