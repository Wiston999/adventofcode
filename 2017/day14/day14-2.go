...
func (p *Problem) AssignRegion(c Coord) {
	if !p.Grid[c].State || p.Grid[c].Region != 0 {
		return
	}
	neighbours := []Coord{
		Coord{c.I - 1, c.J},
	...
	current := p.Grid[c]
	log.Debug(fmt.Sprintf("Assigned region to %v: %d %v", c, p.Regions, current))
	current.Region = p.Regions
	p.Grid[c] = current

	for _, n := range neighbours {
		if cell, ok := p.Grid[n]; ok {
			log.Debug(fmt.Sprintf("Filling neighour %v of %v (%v)", n, c, cell))
			p.AssignRegion(n)
		}
	}
}
...
func solution(context *cli.Context) (result int) {
	...
	problem := Problem{Grid: make(map[Coord]Cell)}
	...
	for i := 0; i < 128; i++ {
		...
		for j, c := range b {
			cell := problem.Grid[Coord{i, j}]
			cell.State = (c == '1')
			problem.Grid[Coord{i, j}] = cell
		}
	}

	problem.Print()
	for coord := range problem.Grid {
		cell := problem.Grid[coord]
		if cell.State && cell.Region == 0 {
			problem.Regions++
			problem.AssignRegion(coord)
		}
	}
	result = problem.Regions
	return
}
